...
filepath = 'Individual PF Results\Clusterpf\';
...
for i=1:numel(gossip_vector)
    for j=1:numel(k_vector)
        ...
        filename = [filepath, 'Track3_Clusterpf'];
%         filename = [filename, '_gossip',num2str(gossip_vector(i))];
    ...
    xtick = [xtick, numel(k_vector)*(i-1)+numel(k_vector)/2+0.5];
    groupSeparator(i) = numel(k_vector)*i+0.5;
    
    colorgroup = [colorgroup, 1:numel(k_vector)];
end
...
for i=1:numel(k_vector)
    ...
    legendText{i} = ['C = ', num2str(k_vector(i))];
end
legend(legendText);
...
xlabel('NGossip');
hold on;
AER_plot = reshape(squeeze(mean(mean(aggregate_error_ratioFull,2),3)), numel(k_vector), numel(gossip_vector));
legendText = {};
for i=1:numel(k_vector)
