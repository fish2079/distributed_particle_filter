...
Again, we omit the curve for $C=1$ as the filter breaks down and yields an RMSE above 12. For $C\geq 6$, the RMSE is quite consistent for $NGossip \geq 6$. As in the case of LApf, Clusterpf is quite robust to gossiping error and yields good performance for $NGossip \geq 5$. 
...
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.49\textwidth]{Figures/Track3_RMSE}}
\end{subfigure}
\begin{subfigure}[RMSE]
{\includegraphics[width=0.49\textwidth]{Figures/Track3_RMSE_focus}}
\end{subfigure}
%\begin{subfigure}[total runtime]
...
%The runtime display identical trends as in the first test track so we focus our analysis on RMSE only.

\begin{figure}
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.55\textwidth]{Figures/Track1_RMSE}}
\end{subfigure}
%\begin{subfigure}[total runtime]
...
Fig.~\ref{fig:Track2_results} shows the tracking results for track 3. BSpf has the best performance. LCpf breaks down and loses the target even with the total overhead of 1200 scalars per time step ($NGossip=60$, 20 scalars to aggregate). CSSpf also has a higher break-off point at $NGossip=60$ (comparing to $NGossip=20$ for track 1). When $NGossip$ is fixed, LApf's performance matches that of BSpf for $m\geq 5$. Conversely, when $m=6$ is fixed, the RMSE of LApf is fairly consistent over all values of $NGossip$. The same trends can be observed in the two curves of Clusterpf. 

\begin{figure}
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.49\textwidth]{Figures/Track2_RMSE}}
\end{subfigure}
\begin{subfigure}[RMSE]
{\includegraphics[width=0.49\textwidth]{Figures/Track2_RMSE_focus}}
\end{subfigure}
%\begin{subfigure}[total runtime]
%{\includegraphics[width=0.65\textwidth]{Figures/Track2_runtime}}
%\end{subfigure}
\caption{Average RMSE with respect to total communication overhead per time step for track 3 ($N=500$, 200 Monte Carlo trials). The subfig b) provides a zoomed view of RMSE curves.}
...
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.75\textwidth]{Figures/RMSE_vs_overhead}}
\end{subfigure}
\begin{subfigure}[RMSE]
{\includegraphics[width=0.75\textwidth]{Figures/RMSE_vs_overhead_zoom}}
\end{subfigure}
\caption{Average RMSE with respect to total communication overhead per time step for track 1 ($N=500$, 200 Monte Carlo trials). The subfig b) provides a zoomed view of RMSE curves.}
\label{fig:Track2_results}
\end{figure}

\section{Sensitivity analysis of particle filters}
Simulation results in the previous sections suggest that LCpf is considerably more susceptible to gossiping noise than the other three filters. In this section, we conduct a sensitivity analysis of the four presented filters to get a better understanding of the presented filters.

We run a centralized BSpf for all three tracks. At each time step, we re-compute the posterior particle weights using all four distributed filters. We then compute and report the discrepancy of approximate and exact particle weights (i.e., $|W_{BS}-W_{CSS}|_2$). We include two implementations of LCpf. In the first implementation, likelihood consensus is used to approximate the measurement model. In the second implementation, likelihood consensus is used to approximate the log-likelihoods directly. All LCpf results from the previous section use the first implementation. 
...
\centering
\begin{subfigure}[Track 1]
{\includegraphics[width=0.69\textwidth]{Figures/curve_perturbation_track3}}
\end{subfigure}
\begin{subfigure}[Track 2]
{\includegraphics[width=0.69\textwidth]{Figures/curve_perturbation_track1}}
\end{subfigure}
\begin{subfigure}[Track 3]
{\includegraphics[width=0.69\textwidth]{Figures/curve_perturbation_track2}}
\end{subfigure}
\caption{Average weight discrepancy between the exact particle weights (BSpf) and approximate particle weights (LCpf, LApf, Clusterpf) with respect to perturbation. A perturbation of $x$ represents $x\%$ shift from the true aggregate value. $N=500$, $m=C=6$, all data points are averaged over 50 time steps and 200 Monte-Carlo trials }
