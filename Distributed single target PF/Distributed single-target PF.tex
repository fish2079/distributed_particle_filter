...
\end{subfigure}
\begin{subfigure}[Gossiping error $\delta_{\text{gossip}}$, solid lines represent true gossiping errors and circles represent the theoretical lower bound]
{\includegraphics[width=0.49\textwidth]{Figures/delta_gossip_track1}}
\end{subfigure}
\begin{subfigure}[Log-likelihood error $\delta$, solid lines represent true errors and dashed lines with circles represent the theoretical higher bound $(1+\delta_m)\delta_{\text{gossip}}+\delta_m$]
...
Finally, Fig.~\ref{fig:detailed_results_track3} shows the results for track 3. With the exception of time step 1, $\delta_m$ remains below unity for all filters. For Clusterpf, the gossiping error remains below 1 even at NGossip = 1. For LApf and LCpf-GS, the gossiping error falls below 1 for NGossip > 3. Finally, for CSSpf and LCpf, much higher communication overhead is required. These trends are also reflected in the overall $\delta$ and normalized particle weights discrepancy. 
...
\end{subfigure}
\begin{subfigure}[Log-likelihood error $\delta$, solid lines represent true errors and circles represent the theoretical higher bound $(1+\delta_m)\delta_{\text{gossip}}+\delta_m$]
