...
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.75\textwidth]{Figures/Track2_bearing_RMSE}}
\end{subfigure}
%\begin{subfigure}[RMSE]
...
Fig.~\ref{fig:Track3_results} shows the tracking results for track 3. Again, LApf and Clusterpf achieve robust tracking performance even with very low communication overhead. LCpf achieves good performance for $NGossip\geq 50$. CSSpf has poor performance even for $NGossip\geq 100$. 

\begin{figure}
\centering
\begin{subfigure}[RMSE]
{\includegraphics[width=0.75\textwidth]{Figures/Track3_bearing_RMSE}}
\end{subfigure}
\caption{Average RMSE with respect to total communication overhead per time step for track 3 ($N=500$, 200 Monte Carlo trials).}
...
Fig.~\ref{fig:perturbation_results} shows the average weight discrepancy with respect to perturbation level. In all three tracks, LCpf has the worst performance by far followed by CSSpf. Even at very lowe perturbation level, the weight discrepancy of LCpf can go above 0.2. On the other hand, LCpf-GS and LApf have consistently the best performance with LApf having an edge for perturbation $>100$. 
...
\end{subfigure}
\caption{Average weight discrepancy between the exact particle weights (BSpf) and approximate particle weights (LCpf, LApf, Clusterpf) with respect to perturbation. A perturbation of $x$ represents $x\%$ shift from the true aggregate value. $N=500$, $d=1,m=C=6$}
