function [mu,sigma] = combinePartResults_avgConsensus(mu_part,sigma_part)
...
mu = P_pow*mu_part; %average the partial means
%average the partial covariances:
sigma =  zeros(size(sigma_part));
%do the averaging for each component separately:
for r = 1:4*parset.numTargets
    for c = 1:4*parset.numTargets
        sigma(r,c,:) = reshape(P_pow*reshape(sigma_part(r,c,:),parset.numSensors,1),[1,1,parset.numSensors]);
    end
end
